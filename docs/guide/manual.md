...
| 項目 | 内容 |
|------|------|
| OS | **Windows 10/11**（`bat/`）または **Linux / macOS**（`sh/`・[linux.md](linux.md)） |
| Python | 3.10 以上推奨 |
| Git | Pages へ push する場合 |
| データベース | `data/kaburadar.db`（別途コピー。Git には含まれない） |
| ネットワーク | 株価取得・GitHub push 時 |

初回セットアップの手順は [セットアップ](setup.md) に従ってください。
...
KabuRadar2/
├── bat/              ← Windows: ここから実行
├── sh/               ← Linux/macOS: ここから実行（[linux.md](linux.md)）
├── config/
│   └── config_lo.ini   ← 戦略・パスなどの設定
├── data/
│   └── kaburadar.db    ← 銘柄・株価データ（自分で置く）
├── output/
│   ├── results/        ← 解析結果 CSV・Excel・集計 CSV
│   └── logs/           ← debug.log
└── docs/
    └── data.json       ← Web 表示用（publish で生成）
```

**触ってよいファイル**

- `bat/*.bat` … 実行用
- `config/config_lo.ini` … パラメータ変更
- `.env` … LINE 通知用（任意・`screening_notify` / `--notify`）

**触らない方がよいもの**

- `src/kaburadar/` … プログラム本体（不具合時は開発者向けドキュメント参照）
- `data/kaburadar.db` … 本番 DB を指している場合はバックアップを取ってから操作
...
1. リポジトリをクローンする  
2. `python -m venv .venv` → 有効化 → `pip install -r requirements.txt`  
3. `data/kaburadar.db` を配置する（[data/README.md](../../data/README.md)）  
4. 動作確認:
...
| やりたいこと | 実行する bat |
|--------------|--------------|
| 株価を更新してから全銘柄を解析 | `bat\screening.bat` |
